...
export async function POST(request: Request) {
  try {
    ...
    const backendResponse = await callMedicalBackend<{ success?: boolean }>("/api/medical/side-effects", data);
    if (backendResponse) {
      return NextResponse.json(backendResponse, { status: backendResponse.success ? 200 : 400 });
    }

    const scriptPath = resolveBridgeScript("run_side_effect_lookup.py");

    if (!scriptPath) {
      return NextResponse.json({ error: "Side-effect bridge script was not found." }, { status: 500 });
    }
    ...
    if (result.status !== 0) {
      return NextResponse.json(
        { error: parsed.error || "Python side-effect workflow failed." },
        { status: 500 }
      );
    }
