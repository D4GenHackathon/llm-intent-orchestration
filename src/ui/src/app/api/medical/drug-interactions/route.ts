...
export async function POST(request: Request) {
  try {
    ...
    const scriptPath = resolveBridgeScript("run_drug_interactions.py");

    if (!scriptPath) {
      return NextResponse.json({ error: "Drug interaction bridge script was not found." }, { status: 500 });
    }
    ...
    if (result.status !== 0) {
      return NextResponse.json(
        { error: parsed.error || "Python drug interaction workflow failed." },
        { status: 500 }
      );
    }
